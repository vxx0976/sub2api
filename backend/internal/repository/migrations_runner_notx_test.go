...
func TestApplyMigrationsFS_NonTransactionalMigration_UsageModelMismatchIndexDropsInvalidIndexBeforeRetry(t *testing.T) {
	...
	mock.ExpectExec("INSERT INTO schema_migrations \\(filename, checksum\\) VALUES \\(\\$1, \\$2\\)").
		...
		WillReturnResult(sqlmock.NewResult(1, 1))
	// dev 把 advisory lock 解锁重构成会话级 conn.QueryRowContext(...).Scan(&unlocked)
	// （session 锁必须同一物理连接加/解），所以是 Query 不是 Exec，与本文件其余 notx 用例一致。
	mock.ExpectQuery("SELECT pg_advisory_unlock\\(\\$1\\)").
		WithArgs(migrationsAdvisoryLockID).
		WillReturnRows(sqlmock.NewRows([]string{"pg_advisory_unlock"}).AddRow(true))
...
func TestApplyMigrationsFS_NonTransactionalMigration_EffectiveModelIndexesDropInvalidIndexesBeforeRetry(t *testing.T) {
	...
	mock.ExpectExec("INSERT INTO schema_migrations \\(filename, checksum\\) VALUES \\(\\$1, \\$2\\)").
		...
		WillReturnResult(sqlmock.NewResult(1, 1))
	// 与本文件其余用例一致：dev 把 advisory unlock 重构成会话级
	// conn.QueryRowContext(...).Scan(&unlocked)（session 锁必须同一物理连接加/解，
	// 且额外校验返回值确认锁真的被释放），所以是 Query 不是 Exec。
	mock.ExpectQuery("SELECT pg_advisory_unlock\\(\\$1\\)").
		WithArgs(migrationsAdvisoryLockID).
		WillReturnRows(sqlmock.NewRows([]string{"pg_advisory_unlock"}).AddRow(true))
...
func TestApplyMigrationsFS_TransactionalMigration(t *testing.T) {
	...
	mock.ExpectQuery("SELECT checksum FROM schema_migrations WHERE filename = \\$1").
		...
		WillReturnError(sql.ErrNoRows)
	mock.ExpectBegin()
	// 运行器在迁移事务内、执行迁移 SQL 前先 SET LOCAL lock_timeout（migrations_runner.go:273）。

...
func expectBackupMigrationUnlock(mock sqlmock.Sqlmock) {
	// dev 把 advisory unlock 重构成会话级 conn.QueryRowContext(...).Scan(&unlocked)
	// （session 锁必须同一物理连接加/解，且额外校验返回值确认锁真的被释放），
	// 所以是 Query 不是 Exec。与 migrations_runner_notx_test.go 的其余用例一致。

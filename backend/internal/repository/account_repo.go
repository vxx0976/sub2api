...
func (r *accountRepository) ResetQuotaUsedAndClearRateLimitCooldown(ctx context.Context, id int64) error {
	result, err := r.sql.ExecContext(ctx,
		...
		) - 'quota_5h_start' - 'quota_daily_start' - 'quota_weekly_start' - 'quota_daily_reset_at' - 'quota_weekly_reset_at',
...
func (r *accountRepository) RevertProxyFallback(ctx context.Context, accountID int64) error {
	...
	res, err := r.sql.ExecContext(ctx, `
		UPDATE accounts SET
			...
			proxy_id=proxy_fallback_origin_id, proxy_fallback_origin_id=NULL, proxy_failure_origin_id=NULL, updated_at=NOW()

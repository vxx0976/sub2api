...
func TestBulkUpdateOpenCodeGoEligiblePredicateIncludesBaseURL(t *testing.T) {
	...
	// fork: BulkUpdate 在 extra = CASE 之前还有 proxy_failure_origin_id = CASE（代理故障回退），
	// 所以锚定在 extra 的 CASE 上，而不是整条 SQL 的第一个 CASE。
	caseStart := strings.Index(query, "extra = CASE")
	require.NotEqual(t, -1, caseStart)
	thenRel := strings.Index(query[caseStart:], "THEN")
	require.NotEqual(t, -1, thenRel)
	firstThen := caseStart + thenRel

...
func (r *groupRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, platform, status, search string, isExclusive *bool, isPurchasable *bool) ([]service.Group, *pagination.PaginationResult, error) {
	...
	return r.listWithFiltersQuery(ctx, q, params, platform, status, search, isExclusive, isPurchasable)
}
...
func (r *groupRepository) ListBindableWithFilters(ctx context.Context, params pagination.PaginationParams, platform, status, search string, isExclusive *bool) ([]service.Group, *pagination.PaginationResult, error) {
	...
	// 可绑定分组列表不暴露 is_purchasable 筛选，传 nil 表示不过滤。
	return r.listWithFiltersQuery(ctx, q, params, platform, status, search, isExclusive, nil)
}

func (r *groupRepository) listWithFiltersQuery(ctx context.Context, q *dbent.GroupQuery, params pagination.PaginationParams, platform, status, search string, isExclusive *bool, isPurchasable *bool) ([]service.Group, *pagination.PaginationResult, error) {
...
// DeleteCascade 保留 fork 的三参形态：migrateToGroupID 非空时先把账号迁到目标分组再删。
// 上游把公共逻辑抽成了 deleteCascade(id, requireEmpty)，这里把迁移参数一并透传下去。
func (r *groupRepository) DeleteCascade(ctx context.Context, id int64, migrateToGroupID *int64) ([]int64, error) {
	return r.deleteCascade(ctx, id, false, migrateToGroupID)
}

func (r *groupRepository) DeleteCascadeIfEmpty(ctx context.Context, id int64) ([]int64, error) {
	return r.deleteCascade(ctx, id, true, nil)
}

func (r *groupRepository) deleteCascade(ctx context.Context, id int64, requireEmpty bool, migrateToGroupID *int64) ([]int64, error) {

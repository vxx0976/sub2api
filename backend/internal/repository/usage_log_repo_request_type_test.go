...
func TestPrepareUsageLogInsert_PersistsImageSizeMetadata(t *testing.T) {
	...
	// 下标随列清单漂移，勿手改：dev 在 ipAddress 与 ImageCount 之间多 country_code（+1），
	// upstream 先加 image_input_tokens / image_input_cost（+2），本轮又加
	// upstream_response_model / upstream_model_mismatch（再 +2）。
	// 当前值由合并后 INSERT 列清单实算：image_size 位于 index 39。
...
func TestUsageLogRepositoryGetStatsWithFiltersAlwaysReturnsAccountCost(t *testing.T) {
	...
	mock.ExpectQuery("FROM usage_logs").
		...
		}).AddRow(int64(50), int64(1000), int64(2000), int64(100), int64(0), int64(0), 15.0, 12.5, 11.0, 100.0))
...
func TestScanUsageLogRequestTypeAndLegacyFallback(t *testing.T) {
	t.Run("image_size_metadata_is_scanned", func(t *testing.T) {
		...
		log, err := scanUsageLog(usageLogScannerStub{values: []any{
			...
			sql.NullString{},
			sql.NullString{},
			sql.NullString{}, // country_code (dev)
			2,

...
func TestGatewayRoutesPinnedModelsDispatchesOrdinaryAndCodexRequests(t *testing.T) {
	...
	// 23 args: fork 在末尾追加了 resellerSettingRepo（上游本轮新增该用例时只有 22 个）。
...
func TestGatewayRoutesRetrievePinnedModel(t *testing.T) {
	...
	upstream := &pinnedModelsRoutesUpstream{}
	cfg := &config.Config{RunMode: config.RunModeSimple}
	// 23 args / 16 args: fork 在两个构造器末尾各追加了一个形参（resellerSettingRepo、
	// 以及 NewGatewayHandler 的 translationService），上游本轮新增该用例时按 22/15 写的。
	s := service.NewOpenAIGatewayService(repo, nil, nil, nil, nil, nil, nil, cfg,
		nil, nil, nil, nil, nil, upstream, nil, nil, nil, nil, nil, nil, nil, nil, nil)
	h := &handler.Handlers{
		Gateway:       handler.NewGatewayHandler(nil, s, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, cfg, nil, nil),

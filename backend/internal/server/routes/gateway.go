...
func RegisterGatewayRoutes(
	...
	// count_tokens 路由：所有 OpenAI 兼容平台(openai/kimi/zhipu/deepseek/minimax)
	...
	// 其余平台（含 OpenAI 兼容的国产供应商 kimi/zhipu/deepseek/minimax 与未解析目标的 composite）
	...
	// (openai/kimi/zhipu/deepseek/minimax)，与 isOpenAIGatewayPlatform 同一口径。
	...
	gateway.Use(requireGroupAnthropic)
	{
		...
		gateway.POST("/embeddings", textBodyLimit, func(c *gin.Context) {
			if !isEmbeddingsCapableGatewayPlatform(c) {
				service.MarkOpsClientBusinessLimited(c, service.OpsClientBusinessLimitedReasonLocalFeatureGate)
				c.JSON(http.StatusNotFound, gin.H{
					"error": gin.H{
						"type":    "not_found_error",

...
func (h *GatewayHandler) GeminiV1BetaListModels(c *gin.Context) {
	...
	if err != nil {
		// 只有强制走 Antigravity 时这份清单才是必需的。纯 Gemini 分组下它只是补充，
		// 调度快照瞬时失败（Redis miss + DB 回退闸门饱和、客户端取消）不应把整个
		// /v1beta/models 打成 503 —— fork 此前就是吞掉该错误继续走 Gemini 账号。
...
func (h *GatewayHandler) GeminiV1BetaModels(c *gin.Context) {
	...
	for {
		...
		submitUsage := func(result *service.ForwardResult) {
			...
			// 长上下文阶梯改由目录数据驱动、统一在计费路径内生效（上游本轮移除了
			// LegacyLongContextRule / RecordUsageWithLongContext 这套入口侧声明 API），
			// 因此这里与其它入口一样只调 RecordUsage。
		...
		if err != nil {
			...
			// 避免上游已产出、客户端已收到内容却整段零计费。PartialError 携带的 error
			// 不是 UpstreamFailoverError，随后直接走终态失败处理，不会 failover 重复计费。

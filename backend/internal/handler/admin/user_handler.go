...
type CreateUserRequest struct {
	...
	Role                 string   `json:"role" binding:"omitempty,oneof=admin user reseller"`
...
func (h *UserHandler) List(c *gin.Context) {
	...
	// Batch lookup parent domains（domainRepo 在测试/无分销配置下可能为 nil，做防御）
	var parentDomains map[int64]string
	if h.domainRepo != nil {
...
func (h *UserHandler) Update(c *gin.Context) {
	...
	// 防锁死保护：管理员不能把自己降级为普通用户/分销商(单管理员场景下会失去后台访问权)。
	// 与既有"不能禁用/删除 admin"保护一致。降级其他管理员仍然允许。
	if (req.Role == service.RoleUser || req.Role == service.RoleReseller) && userID == getAdminIDFromContext(c) {

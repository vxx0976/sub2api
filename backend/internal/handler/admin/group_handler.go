...
func NewGroupHandler(adminService service.AdminService, dashboardService *service.DashboardService, groupCapacityService *service.GroupCapacityService, groupHealthCheckService *service.GroupHealthCheckService) *GroupHandler {
	return NewGroupHandlerWithConfig(adminService, dashboardService, groupCapacityService, groupHealthCheckService, nil)
}

// NewGroupHandlerWithConfig 是 wire 实际使用的构造器（见 internal/handler/wire.go）。
// cfg 只用于运行模式判定（简单模式的能力裁剪）；groupHealthCheckService 是 fork 的分组健康检查依赖。
func NewGroupHandlerWithConfig(adminService service.AdminService, dashboardService *service.DashboardService, groupCapacityService *service.GroupCapacityService, groupHealthCheckService *service.GroupHealthCheckService, cfg *config.Config) *GroupHandler {
	return &GroupHandler{
...
type UpdateGroupRequest struct {
	Name                      string                         `json:"name"`
	Description               *string                        `json:"description"` // nil 不改，&"" 清空
	NameI18n                  map[string]string              `json:"name_i18n"`   // fork 独有：分组名多语言
	DescriptionI18n           map[string]string              `json:"description_i18n"`
	Platform                  string                         `json:"platform" binding:"omitempty,oneof=anthropic openai gemini antigravity grok kimi zhipu deepseek minimax composite"`
	RateMultiplier            *float64                       `json:"rate_multiplier"`
	IsExclusive               *bool                          `json:"is_exclusive"`
...
func (h *GroupHandler) List(c *gin.Context) {
	...
	groups, total, err := h.adminService.ListGroups(c.Request.Context(), page, pageSize, platform, status, search, isExclusive, isPurchasable, sortBy, sortOrder)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *GroupHandler) Delete(c *gin.Context) {
	...
	if h.isSimpleMode() {
		...
		if migrateStr := c.Query("migrate_to_group_id"); migrateStr != "" {
			v, parseErr := strconv.ParseInt(migrateStr, 10, 64)
			if parseErr != nil {

...
func (h *GatewayHandler) Messages(c *gin.Context) {
	...
	if platform == service.PlatformGemini {
		...
		for {
			...
			if err != nil {
				...
				if errors.As(err, &failoverErr) {
					...
					case FailoverExhausted:
						h.incrGroupStatus(c.Request.Context(), apiKey.GroupID, false, reqLog, nil)
						h.handleFailoverExhausted(c, fs.LastFailoverErr, service.PlatformGemini, streamStarted)
						return
					case FailoverCanceled:
						failoverClientGone(c)
			...
			// 分组状态计数：成功
			h.incrGroupStatus(c.Request.Context(), apiKey.GroupID, true, reqLog, nil)
		...
		for {
			...
			if err != nil {
				...
				if errors.As(err, &betaBlockedErr) {
					h.incrGroupStatus(c.Request.Context(), currentAPIKey.GroupID, false, reqLog, nil)
				...
				reqLog.Error("gateway.forward_failed", forwardFailedFields...)
				// ⚠️ 不要在这里再记一次 usage：上游在此处对 result != nil 一律补记，
				// 而 fork 早在本分支开头就按更严的 result.PartialError 记过了
				// （见 submitForwardUsageRecord 的部分交付调用），两处并存会双倍计费。
				// 这里只沿用上游的会话槽语义：上游已接受并计量本次会话，槽位保持不放。
				if result != nil {
			...
			// 分组状态计数：成功
			h.incrGroupStatus(c.Request.Context(), currentAPIKey.GroupID, true, reqLog, nil)
...
func defaultCodexModelIDsForPlatform(platform string) []string {
	...
	case service.PlatformDeepseek:
		return []string{"deepseek-v4-pro", "deepseek-flash"}
...
func (h *GatewayHandler) submitForwardUsageRecord(
	...
	// 客户端显式请求的 effort 与最终生效的 effort 分开记账：上游本轮新增
	// usage_log.requested_reasoning_effort，只在尚未被更靠近上游的一层写过时补。
	stampForwardRequestedReasoningEffort(result, service.NormalizeClaudeOutputEffort(outputEffort))

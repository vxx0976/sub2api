...
func (h *OpenAIGatewayHandler) CountTokens(c *gin.Context) {
	...
	// 与 /v1/messages 派发主路径同一判定（allowOpenAICompatibleMessagesDispatch）：
	// Grok / CN 供应商分组的 AllowMessagesDispatch 会被 sanitizeGroupMessagesDispatchFields
	// 强制清 false，直接读字段会把派发本身可用的分组在 count_tokens 上误拒 403。
	// 上游签名多了 c，用于复合分组解析到的目标平台判定。

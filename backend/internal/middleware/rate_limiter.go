...
func (r *RateLimiter) LimitWithOptions(key string, limit int, window time.Duration, opts RateLimitOptions) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		// 默认按安全客户端 IP 计数；KeyFunc 返回非空时改用自定义维度（如按服务令牌 ID）。
		dimension := clientIPForRateLimit(c)
		if opts.KeyFunc != nil {
		...
		result, err := r.Allow(c.Request.Context(), key+":"+dimension, limit, window)
		if err != nil {
			log.Printf("[RateLimit] redis error: key=%s mode=%s err=%v", r.prefix+key+":"+dimension, failureModeLabel(failureMode), err)
			if failureMode == RateLimitFailClose {

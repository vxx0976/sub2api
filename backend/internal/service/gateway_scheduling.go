...
func (s *GatewayService) SelectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, metadataUserID string, sub2apiUserID int64) (*AccountSelectionResult, error) {
	...
	if len(routingAccountIDs) == 0 && sessionHash != "" && stickyAccountID > 0 && !isExcluded(stickyAccountID) {
		...
		if accountID > 0 && !isExcluded(accountID) {
			...
			if ok {
				...
				if !clearSticky && platformOK && profitOK && modelSupported && modelSchedulable && quotaOK && windowCostOK && dailyCostOK && weeklyCostOK && rpmOK && schedulable {
	...
	if err != nil {
		...
		// 分层过滤选择：优先级 →（可选）最早重置 → 负载率 → 周紧迫度 → LRU
		for len(available) > 0 {
			// 1. 取优先级最小的集合
			...
			if err == nil && accountID > 0 && containsInt64(routingAccountIDs, accountID) {
				if _, excluded := excludedIDs[accountID]; !excluded {
					...
					if err == nil {
						...
						if !clearSticky && s.isGatewayAccountProfitEligible(ctx, account) && s.isAccountInGroup(account, groupID) && account.Platform == platform && (requestedModel == "" || s.isModelSupportedByAccountWithContext(ctx, account, requestedModel)) && s.isAccountSchedulableForModelSelection(ctx, account, requestedModel) && s.isAccountSchedulableForQuota(account) && s.isAccountSchedulableForWindowCost(ctx, account, true) && s.isAccountSchedulableForDailyCost(ctx, account) && s.isAccountSchedulableForWeeklyCost(ctx, account) && s.isAccountSchedulableForRPM(ctx, account, true) && !s.isStickyAccountUpstreamRestricted(ctx, groupID, account, requestedModel) {
							if s.debugModelRoutingEnabled() {
...
func (s *GatewayService) isModelSupportedByAccount(account *Account, requestedModel string) bool {
	...
	// DeepSeek / Moonshot / GLM / Qwen / Seedance：API Key 模式，允许所有模型（由 GetMappedModel 处理）

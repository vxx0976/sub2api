...
type UserListFilters struct {
	...
	// group types since it matches the key's group directly, not allowed_groups.（来自 main）
	APIKeyGroupID int64
	Attributes    map[int64]string // Custom attribute filters: attributeID -> value
	ParentID      *int64           // Filter by parent reseller ID（来自 dev）
...
type UserRepository interface {
	...
	// DeductBalanceIfSufficient 条件原子自减：仅当余额充足时扣减，否则返回
	// ErrInsufficientBalance（经销商生成/扣款路径依赖它，见 reseller_service.go）。
	...
	ListIDsByRole(ctx context.Context, role string) ([]int64, error)
...
func (s *UserService) ChangePassword(ctx context.Context, userID int64, req ChangePasswordRequest) error {
	...
	// dev 的 users 表有真实 token_version 列，因此不能退回 upstream 的
	// Update(..., UserUpdateFields{PasswordHash: true})——那样版本号不落库。
...
func (s *UserService) UpdateStatus(ctx context.Context, userID int64, status string) error {
	...
	// 等价于 upstream 的 Update(..., UserUpdateFields{Status: true})，但省掉一次读。

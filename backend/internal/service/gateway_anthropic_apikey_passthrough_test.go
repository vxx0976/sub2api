...
func TestGatewayService_Forward_UpstreamErrorEventBeforeContent_FailsOver(t *testing.T) {
	...
	svc := &GatewayService{
		...
		responseHeaderFilter: compileResponseHeaderFilter(cfg),
		httpUpstream:         upstream,
		// 必须注入 accountRepo：overloaded_error 会被判成 529，而上游把 handle529
		// 的调用点从 switch 里提前到了各闸门之前，本用例因此首次走到
		// handle529 → accountRepo.SetOverloaded。裸 RateLimitService{} 会 nil deref，
		// 且在网关路径上会被 recoverResponsesPanic 吞成 502 而非测试失败。
		rateLimitService: &RateLimitService{accountRepo: &gatewayForwardErrorPolicyRepoStub{}},
		deferredService:  &DeferredService{},
	}
	...
	// 上游把「尚未产出内容 + error.type=overloaded_error」的 SSE 错误帧按语义映射成 529
	// （并触发过载冷却副作用），取代原先固定 403 的兼容垫片——fork 旧注释也写明 403 只是
	// 「兼容既有 failover/客户端响应语义」。overloaded 本就是 529，这里跟随上游。

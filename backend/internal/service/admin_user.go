...
// 合法值 admin/user/reseller（reseller 为 fork 定制角色）。
func normalizeUserRole(role, fallback string) (string, error) {
	if role == "" {
		return fallback, nil
	}
	if role != RoleAdmin && role != RoleUser && role != RoleReseller {
		return "", fmt.Errorf("invalid role: %q (must be %s, %s or %s)", role, RoleAdmin, RoleUser, RoleReseller)
	}
	return role, nil
}
...
func (s *adminServiceImpl) CreateUser(ctx context.Context, input *CreateUserInput) (*User, error) {
	...
	// 角色可由管理员在创建时指定(admin/user/reseller);未提供时默认 user。

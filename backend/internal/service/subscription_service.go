...
func (s *SubscriptionService) assignOrExtendSubscription(ctx context.Context, input *AssignSubscriptionInput, deferCacheInvalidation bool) (*UserSubscription, bool, error) {
	...
	if existingSub != nil {
		...
		return sub, true, err // true 表示是续期（非新订阅）
	}
...
func (s *SubscriptionService) CheckAndResetWindows(ctx context.Context, sub *UserSubscription, group *Group) error {
	...
	// 新窗口起点由 automaticWindowStartAt 计算（对齐订阅周期锚点，且不越过到期时间）
	if windowStart, ok := sub.automaticWindowStartAt(sub.MonthlyWindowStart, 30*24*time.Hour, now); ok {
		...
		if group != nil && group.HasMonthlyLimit() {
			...
			// CAS 条件重置（expectedWindowStart 作为并发判据），并发下失败方为 no-op
			if err := s.userSubRepo.DeductAndResetMonthlyUsage(ctx, sub.ID, expectedWindowStart, sub.MonthlyUsageUSD, *group.MonthlyLimitUSD, windowStart); err != nil {
				return err
			}
...
func (s *SubscriptionService) ValidateAndCheckLimits(sub *UserSubscription, group *Group) (needsMaintenance bool, err error) {
	...
	//    原地修改还会被并发请求重复叠加。
	check := *sub
	if check.canAutomaticallyResetDailyAt(now) {
		check.DailyUsageUSD = 0
		needsMaintenance = true
	}
	if check.canAutomaticallyResetWeeklyAt(now) {
		check.WeeklyUsageUSD = 0
		needsMaintenance = true
	}
	if check.canAutomaticallyResetMonthlyAt(now) {

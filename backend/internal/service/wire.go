...
// 注意：dev fork 的 NewAuthService 比上游多一个 referralService 参数（推荐返利），
// 这里按 dev 的宽签名透传，captcha provider 仍走 setter 注入。
...
// SetAccountDirectory 是 setter 注入，Wire 生成不出来；手写在 wire_gen.go 里会在重生成时
// 被静默丢掉（插件的 OpenAI 出站身份服务随之恒回 Unavailable），因此收进 provider。
...
func ProvideAccountExpiryService(accountRepo AccountRepository, lockCache LeaderLockCache, db *sql.DB) *AccountExpiryService {
...
func ProvideIdempotencyCleanupService(repo IdempotencyRepository, cfg *config.Config, lockCache LeaderLockCache, db *sql.DB) *IdempotencyCleanupService {
...
// ProvideOAuthRefreshAPI wraps NewOAuthRefreshAPI to avoid wire's variadic
// parameter issue ([]time.Duration not found). Uses default lockTTL.

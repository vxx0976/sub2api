...
func (m *mockGroupRepoForGateway) ListWithFilters(ctx context.Context, params pagination.PaginationParams, platform, status, search string, isExclusive *bool, isPurchasable *bool) ([]Group, *pagination.PaginationResult, error) {
	return nil, nil, nil
}
...
func TestGatewayService_ResolveGatewayGroup_DetectsFallbackCycle(t *testing.T) {
	...
	gotGroup, gotID, _, err := svc.resolveGatewayGroupWithVirtual(ctx, &groupID)
	require.Error(t, err)

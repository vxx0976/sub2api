...
// 由上游同名用例改写。上游断言 CN 分组一律返回空，理由是「openai 默认值会发给 CN 上游」。
// 这个理由本 fork 同样成立，但**解法不同**：上游假定 CN 账号配 api_protocol=anthropic
// 原生直通、模型名交给账号级 model_mapping；本站不满足该前提（生产 Deepseek 分组近 30 天
// 约 90% 请求用 claude-* 模型名，靠分组级映射翻成 deepseek-v4-pro；账号级 model_mapping
...
func TestResolveMessagesDispatchModel_CNProvidersNoOpenAIDefaults(t *testing.T) {
	...
	for platform, exp := range want {
		...
		require.Equal(t, exp[0], g.ResolveMessagesDispatchModel("claude-sonnet-4-5"),
			"CN 分组(%s) 应按平台兜底", platform)
		require.Equal(t, exp[1], g.ResolveMessagesDispatchModel("claude-opus-4-1"), platform)
		for _, m := range []string{"claude-sonnet-4-5", "claude-opus-4-1"} {
			require.NotContains(t, g.ResolveMessagesDispatchModel(m), "gpt-",
				"CN 分组(%s)不得返回 openai 专属型号", platform)
		}
	}

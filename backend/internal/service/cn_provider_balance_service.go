...
func (s *CNProviderBalanceService) queryBalanceForAccount(ctx context.Context, account *Account) (*CNProviderBalanceResult, error) {
	...
	if targetURL == "" {
		// 该账号没有可用的余额端点（如 base_url 指向 Kimi For Coding 或中转）。
		// 明确报「无端点」而不是探一个猜出来的主机——调用方（周期任务 checkOne）
		// 拿到错误后走 NoChange，不会因此改动账号的可调度状态。
		return nil, infraerrors.New(http.StatusBadRequest, "CN_BALANCE_NO_ENDPOINT",
			"account has no balance endpoint for its configured base_url")
	}
	...
	case PlatformKimi:
		...
		//
		// ⚠️ 解析失败**必须**报错返回，不能当成余额 0。下游 checkOne 只看 Success：
		// Success=true + balance 0 < 阈值 0.5 → SetTempUnschedulable 20 分钟，
		// 10 分钟后再探又是 0，线上唯一的 Kimi 账号会被无限循环停调。
		// Kimi For Coding（订阅制）本就没有 available_balance 字段，天然踩这条。
		// 对照 DeepSeek 分支：balance_infos 缺失时 result.Error=... 直接 return，
		// Success 保持 false，checkOne 走 NoChange 不动现状——这里对齐它。
		if code := gjson.GetBytes(bodyBytes, "code"); code.Exists() && code.Int() != 0 {
			result.Error = fmt.Sprintf("Upstream error code %d: %s",
				code.Int(), truncate(strings.TrimSpace(gjson.GetBytes(bodyBytes, "status_msg").String()), 160))
			return result, nil
		}
		raw := gjson.GetBytes(bodyBytes, "data.available_balance")
		if !raw.Exists() {
			result.Error = "Invalid balance response: missing data.available_balance"
			return result, nil
		}
		balance, ok := cnParseF64(raw.Value())
		if !ok {
			result.Error = fmt.Sprintf("Invalid balance response: unparsable data.available_balance (%s)",
				truncate(raw.Raw, 80))
			return result, nil
		}
...
func cnBalanceURL(account *Account) string {
	...
	case PlatformKimi:
		// 余额端点只有 Moonshot 开放平台（api.moonshot.cn）有。账号 base_url 指向别处时
		// 直接不探测：
		//   - Kimi For Coding（api.kimi.com/coding）是订阅制，没有余额概念，探了也只会
		//     拿到 200 + 空 body，白白把 api_key 发给一台运维从未为此配置过的主机；
		//   - 各类中转同理。
		// 返回空串 → 调用方跳过本次探测，账号状态保持不变。
		if base := strings.TrimSpace(account.GetCredential("base_url")); base != "" {
			if u, err := url.Parse(base); err != nil || !strings.EqualFold(u.Host, "api.moonshot.cn") {
				return ""
			}
		}

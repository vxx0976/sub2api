...
func (s *BillingService) CalculateTokenCostForRequest(req TokenCostRequest) (*CostBreakdown, error) {
	...
	if legacyLongContextApplies(resolved, req.Group, req.LegacyLongContext) {
		// 必须用 ...At 变体：官方时段档（DeepSeek 峰谷）要按请求开始冻结的 PricingAt 定档。
		// 非 At 变体等价于传 time.Time{}，恒取基准价（最贵档），谷时会多收。
		return s.CalculateCostWithLongContextAt(req.Model, req.Tokens, req.RateMultiplier,
			req.LegacyLongContext.Threshold, req.LegacyLongContext.Multiplier, req.PricingAt)
	}

...
func TestAuthService_Register_InvitationCodeSingleUseUnderConcurrency(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		go func(i int) {
			...
			_, _, err := svc.RegisterWithVerification(ctx, email, "Password123!", "", "", code, "", nil, "")
			results <- err
		}(i)
	}
	close(start)
...
func TestAuthService_Register_InvitationCodeRejectedWhenAlreadyUsed(t *testing.T) {
	...
	_, _, err := svc.RegisterWithVerification(context.Background(), "later@example.com", "Password123!", "", "", code, "", nil, "")
...
func TestAuthService_Register_InvitationCodeMissingWhenEnabled(t *testing.T) {
	...
	_, _, err := svc.RegisterWithVerification(context.Background(), "no-invite@example.com", "Password123!", "", "", "", "", nil, "")

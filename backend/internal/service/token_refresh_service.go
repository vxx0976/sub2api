...
const (
	...
	defaultTokenRefreshLeaderLockLease          = 10 * time.Minute
)
...
func (s *TokenRefreshService) processRefreshContext(parent context.Context) {
	...
	// 多实例互斥：只有持有 leader lock 的实例执行本轮刷新。
	// 锁在 parent ctx 上获取（而非带 cycle 超时的 ctx），并覆盖整轮生命周期；
	// lease 必须大于本轮最坏运行时长，否则锁会在运行中过期导致并发刷新。
	release, ok := tryAcquireSingletonLeaderLock(parent, s.lockCache, s.db, "leader:token_refresh", s.instanceID, s.leaderLockLease())
	if !ok {
		return
	}
	if release != nil {
...
// leaderLockLease 返回 leader lock 的 TTL（纯崩溃兜底）。
// 正常路径下本轮结束即 release，每轮重新竞选；TTL 只在实例崩溃时兜底。
// 必须 >= 本轮 cycle 超时，否则锁可能在运行中过期，造成多实例并发刷新。
func (s *TokenRefreshService) leaderLockLease() time.Duration {
	lease := s.cycleTimeout() + time.Minute
	if lease < defaultTokenRefreshLeaderLockLease {
		lease = defaultTokenRefreshLeaderLockLease
	}
	return lease
}

...
const (
	...
	// 后 5 个 provider（antigravity/kimi/zhipu/deepseek/minimax）为配额模式引入：
	// antigravity 无探活 adapter（仅配额），其余 4 个复用 OpenAI 兼容探活。
	// ⚠️ 加常量还不够：channel_monitor_validate.go 的 monitorProviders 与
	// probeCapableProviders 两张白名单必须同步补，否则新建监控会被挡成 400。

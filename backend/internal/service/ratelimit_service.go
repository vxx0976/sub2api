...
func (s *RateLimitService) HandleUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte, requestedModel ...string) (shouldDisable bool) {
	...
	case 401:
		...
		if authAccount.Type == AccountTypeOAuth {
			...
			cooldownMinutes := s.cooldownMinutesOrDefault(
				func(c config.RateLimitConfig) int { return c.OAuth401CooldownMinutes }, 10)
...
// cooldownMinutesOrDefault 从配置读冷却分钟数，缺配置或非正值时回落 fallback。
//
// s.cfg 在生产由构造函数注入，但本服务位于网关错误处理路径上：这里 panic 会被
// recoverResponsesPanic 吞成 502，比"用默认冷却时长"糟得多。相邻的 s.settingService
// 本来就是 nil 安全 + 回落默认值，s.cfg 同口径处理。
func (s *RateLimitService) cooldownMinutesOrDefault(pick func(config.RateLimitConfig) int, fallback int) int {
	if s != nil && s.cfg != nil {
		if v := pick(s.cfg.RateLimit); v > 0 {
			return v
		}
	}
	return fallback
...
func (s *RateLimitService) handle529(ctx context.Context, account *Account) {
	...
	// 回退到配置文件
	if settings == nil {
		cooldown := s.cooldownMinutesOrDefault(
			func(c config.RateLimitConfig) int { return c.OverloadCooldownMinutes }, 10)

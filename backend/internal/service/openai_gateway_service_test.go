...
// 复数形态的 ListSchedulable* 仍是 AccountRepository 接口成员（gateway_scheduling /
// scheduler_snapshot_service / gemini_messages_compat_service 在用），OpenAI 调度路径
// 撤掉平台 fan-out 后本文件不再触发它们，但保留桩实现以免误判为「接口没实现」。

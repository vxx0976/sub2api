...
func (s *adminServiceImpl) DuplicateAccount(ctx context.Context, id int64, actorScope, operationKey string) (*Account, error) {
	...
	input := &CreateAccountInput{
		...
		ActiveStartTime:       cloneAccountValuePointer(source.ActiveStartTime),
		ActiveEndTime:         cloneAccountValuePointer(source.ActiveEndTime),
...
func (s *adminServiceImpl) UpdateAccount(ctx context.Context, id int64, input *UpdateAccountInput) (*Account, error) {
	...
	if input.Extra != nil {
		...
		// 保留配额用量和专用服务受管字段，防止普通账号编辑意外覆盖（其余字段允许前端显式覆盖）。
		for _, key := range []string{
			...
			if _, inputHas := normalizedExtra[key]; !inputHas {

...
func TestGetFallbackPricing_FamilyMatching(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "openai legacy gpt5.1 falls back to gpt5.6 sol", model: "gpt-5.1", expectedInput: 5e-6},
		...
		},
		{
			...
			// 与 PricingService.matchDeepSeekCNY 同向：宁可多收也绝不静默少收（kimi-k3 事故）。
			name:              "unknown deepseek maps to most expensive pro tier",
			model:             "deepseek-foo",
			expectedInput:     9e-6,
			expectedOutput:    floatPtr(2.7e-5),
			expectedCacheRead: floatPtr(3e-7),

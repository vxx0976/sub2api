...
func TestCalculateTokenCostContextTierEnablement(t *testing.T) {
	...
	// 与上游的差异点：上游此处传 accountDisabled=false 仍期望 400e-6（账号 false 不否决）。
	// 本 fork 保留否决语义——两侧默认值是「分组恒 true（迁移 222 强制刷）× 账号缺省 false」，
	// 采用上游语义会让绝大多数生产账号的长上下文请求凭空翻倍（实测 0.900 → 1.725）。
	t.Run("account disabled vetoes enabled group", func(t *testing.T) {
		resolved.longContextPricingEnabled = true
...
// 渠道/分组价卡上显式配置的 flex/fast 倍率必须穿过 fork 的 service_tier 准入门。
// 门（见 #3）只挡「隐式」的 flex 五折；FastMultiplier/FlexMultiplier 只可能来自价卡，
// 不豁免的话上游这个功能在它唯一的适用场景（自定价）里永远取不到。
func TestConfiguredChannelTierMultiplierSurvivesServiceTierGate(t *testing.T) {
	resolver := &ModelPricingResolver{}
	tokens := UsageTokens{InputTokens: 1_000_000}

	withFlex := &ResolvedPricing{
		Mode:        BillingModeToken,
		Source:      PricingSourceChannel,
		BasePricing: &ModelPricing{InputPricePerToken: 1e-6, FlexMultiplier: pricingMultiplier(0.8)},
	}
	cost, err := (&BillingService{}).calculateTokenCost(withFlex, CostInput{
		Model:          "glm-4.6",
		Tokens:         tokens,
		RateMultiplier: 1,
		ServiceTier:    "flex",
	...
	// 未配置倍率时准入门照旧生效：自定价不被 flex 打五折。
	withoutFlex := &ResolvedPricing{
		Mode:        BillingModeToken,
		Source:      PricingSourceChannel,
		BasePricing: &ModelPricing{InputPricePerToken: 1e-6},
	}
	cost, err = (&BillingService{}).calculateTokenCost(withoutFlex, CostInput{
		Model:          "glm-4.6",
		Tokens:         tokens,
		RateMultiplier: 1,
		ServiceTier:    "flex",

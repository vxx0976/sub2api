...
type AdminService interface {
	...
	// Group management
	ListGroups(ctx context.Context, page, pageSize int, platform, status, search string, isExclusive *bool, isPurchasable *bool, sortBy, sortOrder string) ([]Group, int64, error)
...
type CreateUserInput struct {
	...
	Username      string
	Notes         string
	Role          string   // "user" or "reseller"; defaults to "user"（来自 dev）
	Balance       *float64 // nil 表示使用系统默认余额（来自 main）
	Concurrency   int
	RPMLimit      int
	AllowedGroups []int64
}
...
type UpdateGroupInput struct {
	Name             string
	Description      *string           // nil 表示不修改；&"" 表示清空（来自 main 的清空修复）
	NameI18n         map[string]string // 来自 dev：多语言
	DescriptionI18n  map[string]string // 来自 dev：多语言
	Platform         string
...
func (s *adminServiceImpl) CreateUser(ctx context.Context, input *CreateUserInput) (*User, error) {
	// Determine role: allow "user" or "reseller", default to "user"（来自 dev）
	role := RoleUser
	if input.Role == RoleReseller {
		role = RoleReseller
	}

	// Balance：显式传值则用之，nil 时回退系统默认余额（来自 main）
	balance := 0.0
	...
	user := &User{
		...
		Notes:         input.Notes,
		Role:          role,    // 来自 dev：支持 user / reseller（绝不会是 admin）
		Balance:       balance, // 来自 main：支持系统默认余额
...
func (s *adminServiceImpl) RefundUserBalance(ctx context.Context, userID int64, amount float64, notes string) error {
	...
	if s.billingCacheService != nil {
		go func() {
			...
			defer cancel()
			if err := s.billingCacheService.InvalidateUserBalance(cacheCtx, userID); err != nil {
				logger.LegacyPrintf("service.admin", "invalidate user balance cache failed (refund): user_id=%d err=%v", userID, err)
			}
		}()
	}
...
func (s *adminServiceImpl) ListGroups(ctx context.Context, page, pageSize int, platform, status, search string, isExclusive *bool, isPurchasable *bool, sortBy, sortOrder string) ([]Group, int64, error) {
	params := pagination.PaginationParams{Page: page, PageSize: pageSize, SortBy: sortBy, SortOrder: sortOrder}
	groups, result, err := s.groupRepo.ListWithFilters(ctx, params, platform, status, search, isExclusive, isPurchasable)
	if err != nil {
		return nil, 0, err
	}
...
func (s *adminServiceImpl) UpdateGroup(ctx context.Context, id int64, input *UpdateGroupInput) (*Group, error) {
	...
	if input.ExternalBuyURL != nil {
		if *input.ExternalBuyURL == "" {
			group.ExternalBuyURL = nil
		} else {
			group.ExternalBuyURL = input.ExternalBuyURL
		}
	}

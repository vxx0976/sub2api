...
type AdminService interface {
	...
	// fork 的 DeleteGroup 支持把账号迁移到另一分组后再删（三参形态，勿退回上游的两参）。
	DeleteGroup(ctx context.Context, id int64, migrateToGroupID *int64) error
	// DeleteGroupIfEmpty 是上游为简单模式加的守卫式删除：仅当分组已无账号绑定时才删。
...
type CreateUserInput struct {
	...
	Username      string
	Notes         string
	Role          string   // 空字符串表示使用默认角色(user);合法值 admin/user/reseller
	Balance       *float64 // nil 表示使用系统默认余额（来自 main）
	Concurrency   int
	RPMLimit      int
	AllowedGroups []int64
	// RestrictPublicGroups 为 true 时，该用户只能绑定 AllowedGroups 中列出的公开分组（来自 main）。
...
type UpdateUserInput struct {
	...
	Username      *string
	Notes         *string
	Role          string   // 空字符串表示"未提供"(不修改);合法值 admin/user/reseller

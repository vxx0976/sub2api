...
// RegisterWithVerification 用户注册（支持邮件验证、优惠码、邀请码、邀请返利码、上级分销商），返回token和用户
func (s *AuthService) RegisterWithVerification(ctx context.Context, email, password, verifyCode, promoCode, invitationCode, affiliateCode string, parentID *int64, registerDomain string) (string, *User, error) {
	...
	if promoCode != "" && s.referralService != nil && s.referralService.IsReferralCode(promoCode) {
		...
		if err != nil {
			// 邀请码验证失败只记录日志，不阻止注册
			logger.LegacyPrintf("service.auth", "[Auth] Failed to validate referral code %s: %v", promoCode, err)
		} else {
			referrerID = &referrer.ID
			logger.LegacyPrintf("service.auth", "[Auth] User registering with referral code from user %d", referrer.ID)
		}
	}
	...
	if s.referralService != nil {
		...
		if err != nil {
			logger.LegacyPrintf("service.auth", "[Auth] Failed to generate referral code: %v", err)
		} else {
	...
	if parentID != nil {
		...
		if err != nil || !parentUser.IsReseller() {
			logger.LegacyPrintf("service.auth", "[Auth] Invalid parent_id %d: user not found or not a reseller", *parentID)
	...
	// 应用优惠码（如果提供且功能已启用，且不是邀请码——邀请码由 referralService
	// 单独处理，走优惠码通道会二次入账）
...
func (s *AuthService) SendVerifyCode(ctx context.Context, email string, opts EmailOptions) error {
	if s.settingService == nil || !s.settingService.IsRegistrationEnabled(ctx) {
	...
	return s.emailService.SendVerifyCode(ctx, email, siteName, opts)
}

// SendVerifyCodeAsync 异步发送邮箱验证码并返回倒计时
func (s *AuthService) SendVerifyCodeAsync(ctx context.Context, email string, opts EmailOptions) (*SendVerifyCodeResult, error) {
	...
	if err := s.emailQueueService.EnqueueVerifyCode(email, siteName, opts); err != nil {
...
func (s *AuthService) RequestPasswordReset(ctx context.Context, email, frontendBaseURL string, locale ...string) error {
	...
	if err := s.emailService.SendPasswordResetEmail(ctx, email, siteName, resetURL, EmailOptions{}); err != nil {
...
// Security: Returns the same response regardless of whether the email exists (prevent user enumeration)
func (s *AuthService) RequestPasswordResetAsync(ctx context.Context, email, frontendBaseURL string, opts EmailOptions) error {
	if !s.IsPasswordResetEnabled(ctx) {
		return infraerrors.Forbidden("PASSWORD_RESET_DISABLED", "password reset is not enabled")
	}
	if s.emailQueueService == nil {
	...
	if err := s.emailQueueService.EnqueuePasswordReset(email, siteName, resetURL, opts); err != nil {

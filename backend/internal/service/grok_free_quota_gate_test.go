...
// dev fork 的 listSchedulableAccounts 在 RunModeSimple 下走的是复数形态
// ListSchedulableByPlatforms（多平台复合分组），上游这个 stub 只实现了单数形态，
// 合并后不补齐会命中内嵌 nil AccountRepository 直接 SIGSEGV。
func (r *grokFreeQuotaAccountRepoStub) ListSchedulableByPlatforms(context.Context, []string) ([]Account, error) {
	return append([]Account(nil), r.accounts...), nil
}

...
func (s *GatewayService) isModelSupportedByAccount(account *Account, requestedModel string) bool {
	...
	// DeepSeek / Moonshot：API Key 模式，允许所有模型（由 GetMappedModel 处理）
	if (account.Platform == PlatformDeepSeek || account.Platform == PlatformMoonshot) && account.Type == AccountTypeAPIKey {
		return true
	}

...
func shouldBillPartialStream(streamResult *streamingResult) bool {
	// usage != nil 不足以判定「已产出内容」：只有 ping 的流也会拿到一个全零 usage，
	// 据此计费会产生零 usage 的幽灵账单记录（对齐上游 partialStreamUsageResult 的
	// hasObservedTokens 门控，见 gateway_upstream_response.go）。

...
// dev fork 的 OpenAIGatewayService.listSchedulableAccounts 走的是复数形态
// （多平台复合分组），上游这个 stub 只实现了单数形态；不补齐会静默落到内嵌
// rateLimitAccountRepoStub 返回空列表，让门控断言假失败。
func (r *thresholdSelectionAccountRepoStub) ListSchedulableByPlatforms(_ context.Context, platforms []string) ([]Account, error) {
	...
	filtered := make([]Account, 0, len(r.accounts))
	for _, account := range r.accounts {
		if _, ok := want[account.Platform]; ok {
			filtered = append(filtered, account)
		}
	}
	return filtered, nil

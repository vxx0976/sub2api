...
func (s *AccountTestService) testCNProviderAnthropicConnection(c *gin.Context, account *Account, modelID string) error {
	...
	testModelID := strings.TrimSpace(modelID)
	if testModelID == "" {
		// 本 fork 已删除 claude.DefaultTestModel（写死的日期快照会 503），统一走
		// getDefaultTestModel(platform) 兜底，见 commit fffab1406。这里的 platform
		// 必为 kimi/zhipu/deepseek，兜底值是各家自己的模型名——把 Claude 的模型名
		// 发给 CN 供应商的 /v1/messages 本来也会被拒。

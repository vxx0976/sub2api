...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	// NormalizeOpenAICompatiblePlatform 现在对 grok/kimi/zhipu/deepseek 原样返回，
	// 精确匹配即可覆盖 CN 供应商；不要再对 account.Platform 二次归一，否则会把
	// 归一到 openai 的其它平台账号误判为匹配。
...
func (s *defaultOpenAIAccountScheduler) selectByLoadBalance(
	...
	for i := range accounts {
		...
		// NormalizeOpenAICompatiblePlatform 对 grok/kimi/zhipu/deepseek 原样返回，
		// 因此 kimi/zhipu/deepseek 账号在精确匹配下天然命中；不要再对
		// account.Platform 二次归一（会让归一到 openai 的其它平台账号误命中）。

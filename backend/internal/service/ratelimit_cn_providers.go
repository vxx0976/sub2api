...
func isCNProviderQuotaExhausted403(account *Account, responseBody []byte, upstreamMsg string) bool {
	...
	// 套餐判定用 effectiveCNAccountMode：生产上有只配了 base_url、没写 account_mode 的
	// 编程套餐号（8a6994351），只看显式 account_mode 会漏掉它们。OpenCode GO 同为窗口型
	// 订阅，与 429 路径、cnProviderQuotaSnapshotReset 保持同一覆盖面。
	codingPlan := account.IsCNProvider() && account.effectiveCNAccountMode() == AccountModeCoding
	if !codingPlan && !account.IsOpenCodeGoPlan() {
		return false
	}

...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput) error {
	...
	usageLog := s.buildRecordUsageLog(ctx, input, result, apiKey, user, account, subscription,
		requestedModel, multiplier, imageMultiplier, accountRateMultiplier, billingType, cacheTTLOverridden, cost, pricingAt)
	...
	usageLog.MerchantRateSnapshot = merchantRateSnapshotLong
	usageLog.PlatformCostSnapshot = platformCostSnapshotLong
	...
	if billingErr != nil {
		...
		// 实际未扣费，ActualCost 归零避免财务口径虚增（TotalCost 保留原值供补录参考）。
...
func (s *GatewayService) calculateTokenCost(
	...
	if s.resolver != nil && apiKey.Group != nil {
		gid := apiKey.Group.ID
		// 必须传 At：上游把预解析结果无条件回传给 CalculateCostUnified，而后者见到
		// Resolved != nil 就不再自行 Resolve（billing_service.go 的「预解析只在命中价卡时
		// 产生」前提已被上游重构打破）。不传 At 会让内置定价路径恒取基准价（最贵档）、
		// 且 TimeBand 为空——DeepSeek 谷时会按峰价多收一倍，priced_at/pricing_time_band
		// 也对不上账。命中渠道价卡时 Resolve 内部会自行把 baseAt 归零，故此处传 At 安全。
		resolved = s.resolver.Resolve(ctx, PricingInput{Model: billingModel, GroupID: &gid, Group: apiKey.Group, At: pricingAt})
	}
...
func (s *GatewayService) buildRecordUsageLog(
	...
	cost *CostBreakdown,
	// pricingAt 是 recordUsageCore 归一化后的计价时刻（零值已回退为 timezone.Now()）。
	// 必须显式传入而不是读 input.PricingAt：后者可能为零值，会让 usage_log.priced_at 为空，
	// 对账时无法把 pricing_time_band 与实际计价时刻对上。
	pricingAt time.Time,
) *UsageLog {
	...
	usageLog := &UsageLog{
		...
		PricedAt:                 optionalTimePtr(pricingAt),
	}
	...
	// 按客户端 IP 解析国家码（IPAddress 本身已在上方结构体字面量中写入）
	if ip := strings.TrimSpace(input.IPAddress); ip != "" {
		if code := geoip.Get().LookupCountry(ip); code != "" {

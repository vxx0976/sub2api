	...
	for _, interval := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			resolved := &ResolvedPricing{
				...
				// fork: 区间路径按渠道配置显式覆盖图片输出价（未配置即归零，见 intervalToModelPricing），
				// 与真实流程 applyTokenOverrides 写进 BasePricing 的值一致；夹具里渠道也要带上同一价格。
				channelPricing: &ChannelModelPricing{ImageOutputPrice: testPtrFloat64(4e-6), TimePricing: &ChannelTimePricing{
	...
	for _, applyLongContext := range []bool{false, true} {
		for _, serviceTier := range []string{"", "priority", "flex"} {
			for _, rateMultiplier := range []float64{-1, 0, 0.8} {
				want, err := bs.calculateCostInternalWithPolicy("gpt-5.4", tokens, rateMultiplier, serviceTier, nil, applyLongContext, time.Time{})
				require.NoError(t, err)

...
func TestBuildVerifyCodeEmailBody_EscapesSiteName(t *testing.T) {
	...
	t.Run("escapes_script_injection", func(t *testing.T) {
		body := svc.buildVerifyCodeEmailBody("123456", `</h1><script>alert(1)</script><h1>`, getEmailTexts("en"))
	...
	t.Run("escapes_html_entities", func(t *testing.T) {
		body := svc.buildVerifyCodeEmailBody("123456", `A&B<C>"D`, getEmailTexts("en"))

		assert.Contains(t, body, "A&amp;B&lt;C&gt;&#34;D")
	})

	t.Run("normal_site_name_unchanged", func(t *testing.T) {
		body := svc.buildVerifyCodeEmailBody("654321", "My Site", getEmailTexts("en"))
...
func TestBuildPasswordResetEmailBody_EscapesSiteName(t *testing.T) {
	...
	t.Run("escapes_html_tags_in_site_name", func(t *testing.T) {
		body := svc.buildPasswordResetEmailBody("https://example.com/reset?token=abc", `</h1><img src=x onerror=alert(1)>`, getEmailTexts("en"))
	...
	t.Run("escapes_html_entities", func(t *testing.T) {
		body := svc.buildPasswordResetEmailBody("https://example.com/reset", `A&B<C>`, getEmailTexts("en"))
	...
	t.Run("normal_site_name_and_url_unchanged", func(t *testing.T) {
		...
		body := svc.buildPasswordResetEmailBody(resetURL, "Sub2API", getEmailTexts("en"))
	...
	t.Run("escapes_ampersand_in_reset_url", func(t *testing.T) {
		...
		body := svc.buildPasswordResetEmailBody(resetURL, "Site", getEmailTexts("en"))

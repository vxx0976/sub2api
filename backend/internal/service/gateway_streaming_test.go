...
func TestHandleStreamingResponse_SSEErrorEvent_ReturnsTypedErrorWithRawData(t *testing.T) {
	...
	require.Error(t, err)
	// dev 的 handleStreamingResponse 改为返回部分交付结果（供上层按 firstTokenMs/usage 决定是否对已交付
	// token 计费）。立即出现的 SSE error 未产出首 token，故不应被部分计费——由上层走 failover。
	require.False(t, shouldBillPartialStream(result), "立即 SSE error（未产出首 token）不应被部分计费，应由上层走 failover")

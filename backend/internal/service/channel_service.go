...
type ChannelService struct {
	...
	// accountRepo 是 fork 扩展：模型广场在渠道侧零模型时回落到账号池推导模型目录
	// （见 channel_plaza_account_fallback.go）。可为 nil（测试场景 / 未接线）。
...
func NewChannelService(repo ChannelRepository, groupRepo GroupRepository, authCacheInvalidator APIKeyAuthCacheInvalidator, pricingService *PricingService, accountRepo AccountRepository, cachePubSub ChannelCachePubSub) *ChannelService {
	s := &ChannelService{
		repo:                 repo,
		groupRepo:            groupRepo,
...
// 两份会各自漂移）；集合内容与**次序**的理由见该函数注释——次序是
// lookupPricingAcrossPlatforms / lookupMappingAcrossPlatforms「首个命中」的行为契约。

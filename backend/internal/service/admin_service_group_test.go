...
func (s *groupRepoStubForAdmin) ListWithFilters(_ context.Context, params pagination.PaginationParams, platform, status, search string, isExclusive *bool, isPurchasable *bool) ([]Group, *pagination.PaginationResult, error) {
...
func (s *groupRepoStubForAdmin) ListBindableWithFilters(ctx context.Context, params pagination.PaginationParams, platform, status, search string, isExclusive *bool) ([]Group, *pagination.PaginationResult, error) {
	// bindable 变体是上游为简单模式加的，没有 fork 的 isPurchasable 形参。
	return s.ListWithFilters(ctx, params, platform, status, search, isExclusive, nil)
}
...
func TestAdminServiceSimpleModeListUsesRepositoryFilteredTotal(t *testing.T) {
	...
	svc := &adminServiceImpl{cfg: &config.Config{RunMode: config.RunModeSimple}, groupRepo: repo}
	groups, total, err := svc.ListGroups(context.Background(), 2, 1, "", "", "", nil, nil, "id", "asc")
	require.NoError(t, err)

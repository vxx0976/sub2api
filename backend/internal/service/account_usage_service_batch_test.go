...
func (r *usageBatchLogRepoStub) BackfillCountryCode(context.Context, string, string) (int64, error) {
	return 0, nil
}
...
// 以下为 dev fork 在 UsageLogRepository 上独有的方法（上游没有），
// 上游新增的这个 stub 未实现，合并后必须补齐否则 service 包测试无法编译。
func (r *usageBatchLogRepoStub) SumUsageCostsByDay(context.Context, time.Time, time.Time, string) (map[string]usagestats.DailyUsageCost, error) {
	return nil, nil
}
func (r *usageBatchLogRepoStub) GetGeoDistribution(context.Context, time.Time, time.Time) ([]GeoDistributionItem, error) {
	return nil, nil
}
func (r *usageBatchLogRepoStub) GetDistinctIPsWithoutCountry(context.Context, int) ([]string, error) {
	return nil, nil
}
func (r *usageBatchLogRepoStub) SumCommissionByUserIDs(context.Context, []int64) (float64, error) {
	return 0, nil
}
func (r *usageBatchLogRepoStub) SumTodayCostByUserIDs(context.Context, []int64) (float64, error) {
	return 0, nil
}
func (r *usageBatchLogRepoStub) BackfillMerchantRateSnapshot(context.Context) (int64, error) {
	return 0, nil
}

...
func TestEmailQueueTasksPreserveLocaleHints(t *testing.T) {
	...
	require.NoError(t, queue.EnqueueVerifyCode("user@example.com", "Sub2API", EmailOptions{Locale: "zh-CN"}))
	require.NoError(t, queue.EnqueuePasswordReset("user@example.com", "Sub2API", "https://example.com/reset", EmailOptions{Locale: "en-US"}))

	verifyTask := <-queue.taskChan
	require.Equal(t, TaskTypeVerifyCode, verifyTask.TaskType)
	require.Equal(t, "zh-CN", verifyTask.Options.Locale)

	resetTask := <-queue.taskChan
	require.Equal(t, TaskTypePasswordReset, resetTask.TaskType)
	require.Equal(t, "en-US", resetTask.Options.Locale)
}

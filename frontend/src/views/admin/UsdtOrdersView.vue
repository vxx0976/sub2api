  ...
  <AppLayout>
    <div class="space-y-6">
      <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ t('admin.usdtOrders.title') }}</h1>
          <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ t('admin.usdtOrders.description') }}</p>
        </div>
      </div>
          ...
          <table class="w-full">
            <thead>
              <tr class="border-b border-gray-100 dark:border-dark-700">
                ...
                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.usdtOrders.usdtAmount') }}</th>
                ...
                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.usdtOrders.txHash') }}</th>
            ...
            <tbody>
              <tr v-for="item in items" :key="item.id" class="border-b border-gray-50 dark:border-dark-800">
                ...
                <td class="px-4 py-3"><span class="text-sm font-mono text-gray-900 dark:text-white">{{ item.usdt_amount_str }} USDT</span></td>
                ...
                <td class="px-4 py-3">
                  <span v-if="item.trade_no" class="text-xs font-mono text-gray-500 dark:text-gray-400">{{ shortHash(item.trade_no) }}</span>
...
import type { AdminUsdtOrderItem } from '@/api/admin/usdtOrders'
...
const items = ref<AdminUsdtOrderItem[]>([])
...
async function loadOrders() {
  ...
  try {
    ...
    const result = await adminAPI.usdtOrders.getAdminUsdtOrders(params)
...
const refundingOrder = ref<AdminUsdtOrderItem | null>(null)
const refundReason = ref('')
const refundSubmitting = ref(false)

function openRefund(item: AdminUsdtOrderItem) {
...
async function confirmRefund() {
  ...
  try {
    await adminAPI.usdtOrders.refundAdminUsdtOrder(refundingOrder.value.order_no, refundReason.value)

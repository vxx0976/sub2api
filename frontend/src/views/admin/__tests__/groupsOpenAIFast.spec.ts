...
// NOTE(fork): upstream imports the nested `@/i18n/locales/<lang>/admin/overview`
// modules. This fork keeps flat per-locale files, so the same keys live at
// `<lang>.admin.groups.openaiFast`. `ru` is fork-only and must be covered too —
// a missing ru key renders the literal key string to users and crawlers.
...
describe("groupsOpenAIFast", () => {
  ...
  it("provides localized switch labels", () => {
    for (const messages of [zh, en, ru]) {
      expect(messages.admin.groups.openaiFast).toMatchObject({

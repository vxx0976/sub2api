...
describe("admin SettingsView email domain quota copy", () => {
  it("documents the email domain quota and empty-whitelist behavior in both locales", () => {
    expect(zhLocale.auth.emailDomainRegistrationLimit).toContain("主流邮箱");
    expect(zhLocale.auth.emailDomainRegistrationLimit).toContain("联系客服");
    expect(enLocale.auth.emailDomainRegistrationLimit).toContain("mainstream email");
    expect(enLocale.auth.emailDomainRegistrationLimit).toContain("contact support");

    // 白名单 hint 描述严格默认语义；额度语义移入独立开关的 hint。
    const zhWhitelistHint = zhLocale.admin.settings.registration.emailSuffixWhitelistHint;
    const enWhitelistHint = enLocale.admin.settings.registration.emailSuffixWhitelistHint;
    expect(zhWhitelistHint).toContain("留空则不限制");
    expect(enWhitelistHint).toContain("leave empty for no restriction");

    const zhQuotaHint = zhLocale.admin.settings.registration.emailDomainQuotaHint;
    const enQuotaHint = enLocale.admin.settings.registration.emailDomainQuotaHint;
...
describe("admin SettingsView platform quota matrix", () => {
  ...
  it("保存时 updateSettings payload 应包含嵌套 default_platform_quotas 对象（含全 5 平台）", async () => {
    ...
    const platforms = ["anthropic", "openai", "gemini", "antigravity", "grok", "deepseek", "moonshot", "glm", "qwen", "seedance"];

...
describe('admin AccountsView — 账号行展示', () => {
  ...
  it('replaces a Grok row when auto refresh returns a changed canonical usage snapshot', async () => {
    ...
    // fork（e4a6e57ff）在首次 lite 加载后会立即再拉一次非 lite 数据补全费用列，
    // 这一次会先消费掉一个 listWithEtag 响应。给它喂初始快照，
    // 让随后由定时器驱动的那次才拿到变更后的快照——用例意图（Free → 刷新 → SuperGrok）不变。
    listWithEtag.mockResolvedValueOnce({
      notModified: false,
      etag: 'grok-snapshot-1',
      data: { items: [initialAccount], total: 1, page: 1, page_size: 20, pages: 1 },
    })
    listWithEtag.mockResolvedValueOnce({
      notModified: false,
    ...
    // 2 次而非上游的 1 次：首次 lite 加载后 fork 会补一次非 lite 刷新（见上），
    // 第 2 次才是定时器驱动的自动刷新。

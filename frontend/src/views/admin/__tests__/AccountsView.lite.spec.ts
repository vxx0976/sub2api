...
describe('admin AccountsView lite account list', () => {
  ...
  it('keeps lite=1 on the initial list request', async () => {
    // fork 差异：params 是复用的响应式对象，首屏之后会被删掉 lite，
    // 因此断言必须在调用发生的当刻取快照，不能事后读同一个引用。
    const seenParams: Record<string, unknown>[] = []
    listAccounts.mockImplementation(async (_page: number, _size: number, params: Record<string, unknown>) => {
      seenParams.push({ ...params })
      return { items: [listRow], total: 1, page: 1, page_size: 20, pages: 1 }
    })
    const wrapper = mountView()
    await flushPromises()

    expect(seenParams[0]).toMatchObject({ lite: '1' })
    wrapper.unmount()
  })
  ...
  // fork 差异：首屏之后刻意走非 lite，用一次全量刷新补回日/周费用聚合
  // （lite 模式后端跳过这两项 PostgreSQL 聚合，见 account_handler 的 lite 分支）。
  it('drops lite on automatic ETag refreshes so cost aggregates are filled', async () => {
    vi.useFakeTimers()
    ...
    const refreshParams = listWithEtag.mock.calls[listWithEtag.mock.calls.length - 1][2] as Record<string, unknown>
    expect(refreshParams.lite).toBeUndefined()
    wrapper.unmount()
  })

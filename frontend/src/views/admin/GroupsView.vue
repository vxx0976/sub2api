...
const allColumns = computed<Column[]>(() => {
  ...
  return [
    ...
    // fork 独有：分组健康度列（上游没有），保持在 capacity 与 usage 之间
...
// Composite routes only target the platforms the backend composite router supports.
...
const handleUpdateGroup = async () => {
  ...
  // 「打开已开白名单的分组、候选还在转圈就点保存」永远存不进去（含改名等无关字段）。
  if (
  ...
  try {
    ...
    payload.image_rate_multiplier = normalizeRateMultiplier(
      payload.image_rate_multiplier as number | string | null | undefined,
    );
    ...
    payload.batch_image_discount_multiplier = normalizeRateMultiplier(
      payload.batch_image_discount_multiplier as number | string | null | undefined,
    );
    payload.batch_image_hold_multiplier = normalizeRateMultiplier(
      payload.batch_image_hold_multiplier as number | string | null | undefined,
    );
    payload.video_rate_multiplier = normalizeRateMultiplier(
      payload.video_rate_multiplier as number | string | null | undefined,
    );
    // 媒体价格输入清空时 v-model.number 产生 ""，直接提交会被后端 *float64 反序列化拒绝（400）。
...
const openSortModal = async () => {
  try {
    ...
    // 按 sort_order 排序
    sortableGroups.value = ([...allGroups] as AdminGroup[]).sort((a, b) => a.sort_order - b.sort_order)

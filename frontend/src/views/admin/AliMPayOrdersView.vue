  ...
  <AppLayout>
    <div class="space-y-6">
      <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ t('admin.alimpayOrders.title') }}</h1>
          <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ t('admin.alimpayOrders.description') }}</p>
        </div>
      </div>
      ...
      <div class="card p-4">
        <div class="flex flex-wrap gap-3">
          ...
          <button @click="resetFilters" class="btn btn-secondary">{{ t('common.reset') }}</button>
        </div>
      </div>
          ...
          <table class="w-full">
            <thead>
              <tr class="border-b border-gray-100 dark:border-dark-700">
                ...
                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.alimpayOrders.paymentAmount') }}</th>
                ...
                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.alimpayOrders.sourceDomain') }}</th>
            ...
            <tbody>
              <tr v-for="item in items" :key="item.id" class="border-b border-gray-50 dark:border-dark-800">
                <td class="px-4 py-3"><span class="text-sm text-gray-900 dark:text-white">{{ item.order_no }}</span></td>
                <td class="px-4 py-3">
                  <div>
                ...
                <td class="px-4 py-3"><span class="text-sm font-medium text-gray-900 dark:text-white">&yen;{{ item.amount }}</span></td>
                <td class="px-4 py-3"><span class="text-sm text-gray-900 dark:text-white">&yen;{{ item.payment_amount }}</span></td>
                <td class="px-4 py-3"><span class="text-sm font-medium text-gray-900 dark:text-white">${{ item.credit_amount }}</span></td>
                <td class="px-4 py-3">
                  <span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium" :class="statusClass(item.status)">
                    {{ statusLabel(item.status) }}
                  </span>
                </td>
                <td class="px-4 py-3"><span class="text-sm text-gray-600 dark:text-gray-300">{{ item.source_domain || '—' }}</span></td>
                <td class="px-4 py-3"><span class="text-sm text-gray-500 dark:text-gray-400">{{ formatDateTime(item.created_at) }}</span></td>
                <td class="px-4 py-3"><span class="text-sm text-gray-500 dark:text-gray-400">{{ item.paid_at ? formatDateTime(item.paid_at) : '—' }}</span></td>
...
import type { AdminAliMPayOrderItem } from '@/api/admin/alimpayOrders'
...
const items = ref<AdminAliMPayOrderItem[]>([])
...
async function loadOrders() {
  ...
  try {
    ...
    const result = await adminAPI.alimpayOrders.getAdminAliMPayOrders(params)
...
const refundingOrder = ref<AdminAliMPayOrderItem | null>(null)
const refundReason = ref('')
const refundSubmitting = ref(false)

function openRefund(item: AdminAliMPayOrderItem) {
...
async function confirmRefund() {
  ...
  try {
    await adminAPI.alimpayOrders.refundAdminAliMPayOrder(refundingOrder.value.order_no, refundReason.value)

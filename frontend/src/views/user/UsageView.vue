          ...
          <template #cell-cost="{ row }">
            <div class="flex items-center gap-1.5 text-sm">
              <span class="font-medium text-green-600 dark:text-green-400">
                {{ costSymbol(row.price_currency) }}{{ (row.actual_cost ?? 0).toFixed(6) }}
              </span>
              <!-- Cost Detail Tooltip -->
              <div
                class="group relative"
      ...
      >
        <div class="space-y-1.5">
          ...
          <div class="flex items-center justify-between gap-6">
            ...
            <span class="font-medium text-white">{{ costSymbol(tooltipData?.price_currency) }}{{ tooltipData?.total_cost.toFixed(6) }}</span>
          </div>
          <div class="flex items-center justify-between gap-6 border-t border-gray-700 pt-1.5">
            <span class="text-gray-400">{{ t('usage.billed') }}</span>
            <span class="font-semibold text-green-400"
              >{{ costSymbol(tooltipData?.price_currency) }}{{ tooltipData?.actual_cost.toFixed(6) }}</span
            >
          </div>
        </div>
        <!-- Tooltip Arrow (left side) -->

...
// Column-settings tests exercise the full toggleable column set (rate_limit/expires_at),
// which the fork gates behind a non-regular (admin/reseller) role. Mock as admin.
vi.mock('@/stores/auth', () => ({
  useAuthStore: () => ({
    isAdmin: true,
    isReseller: false,
...
describe('user KeysView column settings', () => {
  ...
  describe('create provider selection', () => {
    ...
    it('resets provider and group when reopening create, and preserves edit options', async () => {
      ...
      await wrapper.get('[data-tour="keys-create-btn"]').trigger('click')
      expect(optionIds(wrapper)).toEqual([1])
      // fork 的 openCreateModal 会预选第一个按量（standard）分组并把厂商页签对齐过去，
      // 所以重开后不是空值而是回到这个默认分组。上游没有这段预选逻辑，故断言 null。
      expect(groupSelect(wrapper).props('modelValue')).toBe(1)
      await wrapper.get('[data-test="close-dialog"]').trigger('click')
      await getButtonByText(wrapper, 'common.edit').trigger('click')

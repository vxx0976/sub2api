            ...
            <div class="group/dropdown relative">
              ...
              >
                <template v-if="row.group">
                  ...
                  />
                  <span
                    v-if="row.group.health_status === 'available'"
                    class="h-2 w-2 rounded-full bg-emerald-500 animate-pulse"
                    :title="t('keys.groupHealthAvailable')"
                  />
                  <span
                    v-else-if="row.group.health_status === 'unavailable'"
                    class="h-2 w-2 rounded-full bg-red-500"
                    :title="t('keys.groupHealthUnavailable')"
                  />
                </template>
...
const groupFilterOptions = computed(() => [
  ...
  ...groups.value.map((g) => ({ value: g.id, label: resolveI18n(g.name_i18n, g.name) }))
])
...
const groupOptions = computed(() =>
  groups.value.map((group) => ({
    value: group.id,
    label: resolveI18n(group.name_i18n, group.name),
    description: resolveI18n(group.description_i18n, group.description || ''),
...
const openCreateModal = () => {
  ...
  if (paygoGroup) {
    ...
    // 上游 0.2.5 给创建弹窗加了厂商页签，分组下拉只显示当前页签下的分组，
    // 且下面那个 watcher 会把「不在当前页签里的 group_id」清空。
    // 不把页签一起对齐，fork 这条「默认选中第一个按量分组」就会被清掉，
    // 用户打开弹窗看到的是空分组。
    createProvider.value = getKeyGroupProvider(paygoGroup.platform)
  }

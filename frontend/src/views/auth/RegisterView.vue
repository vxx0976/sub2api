...
const promoCodeEnabled = ref<boolean>(
  // 经销商域名下恒不展示平台返利码（与下方 loadPublicSettings 的判定同口径）。
  appStore.cachedPublicSettings?.promo_code_enabled === true && !appStore.isResellerDomain
)
...
onMounted(async () => {
  ...
  try {
    ...
    // siteName is a computed backed by appStore (dev fork branding); no assignment here.

export default {
  ...
  modelPlaza: {
    ...
    table: {
      ...
      timeTierNoteWeekdays: 'Official time-based pricing: peak {windows} ({tz}) on weekdays only; {percent}% of that rate at all other times and all weekend'
    },
    nav: {
      login: 'Sign In',
  ...
  admin: {
    ...
    users: {
      ...
      disableKey: 'Disable',
      enableKey: 'Enable',
      keyDisabledSuccess: 'Key disabled, effective immediately',
      keyEnabledSuccess: 'Key enabled',
      keyStatusChangeFailed: 'Failed to change key status',
      group: 'Group',
      none: 'None',
    ...
    groups: {
      ...
      openaiMessages: {
        ...
        haikuModelPlaceholder: 'e.g., gpt-5.6-sol',
    ...
    promptAudit: {
      ...
      saveBar: { enabled: 'Enable prompt audit', blocking: 'Synchronous blocking', blockingLatestTurnOnly: 'Only latest input and prior output', blockingLatestTurnOnlyHint: 'Applies to the synchronous blocking path: the block decision inspects only the latest user turn and the preceding model output, ignoring earlier history. Effective only while synchronous blocking is on.', auditLatestTurnOnly: 'Async audit scans latest turn only', auditLatestTurnOnlyHint: 'Applies to the asynchronous audit path: only the latest user turn and the preceding model output are sent to the guard, so long contexts (such as requests carrying tens of thousands of characters of code) are no longer scanned in full, cutting audit compute sharply. Archiving is unaffected: the complete conversation is still stored and reviewable. Effective only while synchronous blocking is off (asynchronous audit mode).', storePass: 'Store safe events', dirty: 'Unsaved changes', synced: 'Configuration synced' },
      blockingConfirm: {

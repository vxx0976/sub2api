export default {
  ...
  reseller: {
    ...
    serviceTokens: {
      title: 'API 令牌',
      description: '用于你的后端自动化调用 key 管理接口（重置配额、启用/禁用、创建 key），无需登录后台。',
      create: '签发新令牌',
      name: '备注名称',
      namePlaceholder: '例如：billing-backend',
      expiresInDays: '有效期（天）',
      expiresInDaysHint: '留空或填 0 表示永不过期',
      neverExpires: '永不过期',
      issue: '签发',
      createdTitle: '令牌已签发',
      createdWarning: '请立即复制保存，令牌明文仅显示这一次，关闭后无法再次查看。',
      copy: '复制',
      copied: '已复制',
      never: '从未使用',
      statusActive: '有效',
      statusRevoked: '已吊销',
      revoke: '吊销',
      revokeConfirm: '吊销后使用该令牌的后端调用将立即失效，确定吊销吗？',
      revokeSuccess: '令牌已吊销',
      empty: '暂无 API 令牌',
      usageTitle: '如何使用',
      usageHint: '在请求头携带 X-Reseller-Token，调用以下接口（仅能操作你自己的 key）：',
      usageResetComment: '客户付款后：重置配额（用满额度自动禁用的 key 会一并复活）',
      usageEnableComment: '按需启用 key',
      columns: {
        name: '备注',
        prefix: '前缀',
        status: '状态',
    ...
    settings: {
      ...
      accessSection: '访问控制',
      accessHint: '控制本商户站点的注册与登录入口',
      disableRegistration: '关闭注册',
      disableRegistrationHint: '开启后，本站点将拒绝新用户注册',
      disableLogin: '关闭登录',
      disableLoginHint: '开启后，本站点的下级用户将无法登录（不影响商户/管理员）',
      contactSection: '联系方式',
      contactWechat: '微信号',

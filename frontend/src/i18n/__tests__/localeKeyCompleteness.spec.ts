...
// 本分支的 en/zh 是「扁平基础文件 + <locale>.custom 覆盖层」两段式，
// i18n/index.ts 在运行时 deepMerge 后才交给 vue-i18n。
// 这里必须用同一套合并逻辑，否则只定义在 custom 层的 key（docs / reseller /
// consoleHome / status 等 fork 独有页面）会被误判为缺失。
// 注意：ru 是 fork 独有语种、没有 custom 覆盖层，且相对 en 仍有历史欠账，
// 因此暂未纳入下面的硬断言；新增 key 仍必须手工同步补进 ru.ts。
function deepMergeMessages(base: LocaleValue, custom: LocaleValue): LocaleValue {
  const result: LocaleValue = { ...base }
  for (const key of Object.keys(custom)) {
    const baseValue = result[key]
    const customValue = custom[key]
    if (
      baseValue && typeof baseValue === 'object' && !Array.isArray(baseValue) &&
      customValue && typeof customValue === 'object' && !Array.isArray(customValue)
    ) {
      result[key] = deepMergeMessages(baseValue as LocaleValue, customValue as LocaleValue)
    } else {
...
const en = deepMergeMessages(enBase as LocaleValue, enCustom as LocaleValue)
const zh = deepMergeMessages(zhBase as LocaleValue, zhCustom as LocaleValue)

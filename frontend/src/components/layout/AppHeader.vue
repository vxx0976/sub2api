<template>
  <header class="glass sticky top-0 z-30 border-b border-gray-200/50 dark:border-dark-700/50">
    <div class="flex h-16 items-center justify-between gap-2 px-2 sm:px-4 md:px-6">
      ...
      <!-- Right: Announcements + Docs + Redeem + Language + Subscriptions + Balance + User Dropdown -->
      <div class="flex min-w-0 items-center gap-1 sm:gap-3">
        ...
        <!-- Subscription Progress (hide for admin; not mounted at all when the feature is off) -->
        <SubscriptionProgressMini v-if="user && !isAdmin && subscriptionFeatureEnabled" />
...
// dev 使用站内 /docs 路由入口，不读取站点设置里的外链文档地址（doc_url）。
// 注意：layout 下的净化用例以「本文件源码不得出现该外链字段的驼峰标识符」固化此不变式，
// 故此处刻意只写 snake_case 形式，勿改回驼峰。

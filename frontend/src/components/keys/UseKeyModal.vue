  ...
  >
    <div class="space-y-4">
      ...
      <template v-else>
        ...
        <div v-if="clientTabs.length > 1" class="overflow-x-auto border-b border-gray-200 dark:border-dark-700">
...
const clientTabs = computed((): TabConfig[] => {
  ...
  switch (props.platform) {
    ...
    case 'minimax':
    case 'composite':
      // NOTE(fork): upstream also lists an OpenCode tab here, but this fork dropped upstream's
      // global OpenCode dispatch in currentFiles (commit 13b3776de) and only kept per-platform
      // branches for openai/grok. Without a deepseek/composite branch the tab would silently
      // render the Claude Code config, so it stays off — same call as gemini/antigravity/default.
      return [
        { id: 'claude', label: t('keys.useKeyModal.cliTabs.claudeCode'), icon: TerminalIcon },
        { id: 'codex', label: t('keys.useKeyModal.cliTabs.codexCli'), icon: TerminalIcon }
      ]
  ...
  switch (props.platform) {
    case 'openai':
      ...
      if (activeClientTab.value === 'opencode') {
        // @ai-sdk/openai appends /responses to baseURL, so it needs the /v1 root too.

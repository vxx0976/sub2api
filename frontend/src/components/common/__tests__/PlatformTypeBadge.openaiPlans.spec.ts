...
describe('PlatformTypeBadge ChatGPT plan tiers', () => {
  ...
  it('falls back to the raw value for unknown plans', () => {
    ...
    // fork 的 planLabel 对未知档位不是原样返回，而是走 enterprise/business 关键字
    // 与 Title Case 兜底（见 PlatformTypeBadge.vue 尾部），所以这里断言 'Enterprise'。
    // 上游那版返回小写原值，改回去会让所有未知档位在徽章里显示成原始下划线串。

...
describe('AccountUsageCell', () => {
  ...
  // fork：原生 opencode_go 不在此列，见下方「原生 opencode_go 仍走 CN 子单元格」用例
  it.each(['kimi', 'zhipu', 'deepseek', 'minimax'] as const)(
  ...
  // fork：CN 后台循环会定时采集原生 opencode_go 的 5h/周/月快照并驱动阈值停调，
  // 即使上游判定 eligible 也要继续展示 CN 额度格，不能被 OpenCode 用量单元格顶掉。
  it('原生 opencode_go 即使 OpenCode Go eligible 也仍走 CN 子单元格', async () => {
    const wrapper = mount(AccountUsageCell, {
      props: {
        account: makeAccount({
          id: 9103,
          ...
          opencode_go_usage: makeOpenCodeGoUsage(9103)
        })
      },
      global: {
        stubs: { ...cnUsageCellStubs, UsageProgressBar: true, AccountQuotaInfo: true }

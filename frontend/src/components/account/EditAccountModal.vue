<template>
  ...
  >
    ...
    >
      ...
      <!-- fork：原生 opencode_go 由 CN 后台循环独占采集，这里不再提供第二个轮询开关（见 AccountUsageCell） -->
      <section
        v-if="account?.platform !== 'opencode_go' && account?.opencode_go_usage?.eligible"
        class="space-y-4 border-t border-gray-200 pt-4 dark:border-dark-600"
...
// 只看平台、不看账号类型：模板里的占位符/默认 base_url 回落用它，
// 避免每加一个国产平台就要在四处 `kimi || zhipu || deepseek` 字面量里补一遍
// —— 漏补的后果是该平台账号的 base_url 回落到 api.anthropic.com，
// 保存后把国产密钥发给 Anthropic。
// 只看平台、不看账号类型：模板里的占位符/默认 base_url 回落用它，
// 避免每加一个国产平台就要在四处 `kimi || zhipu || deepseek` 字面量里补一遍
// —— 漏补的后果是该平台账号的 base_url 回落到 api.anthropic.com，
// 保存后把国产密钥发给 Anthropic。
// opencode_go 不在 isCNProviderPlatform 里（它不是国产供应商），但 base_url 预设
// 同样走 defaultCNBaseUrl，所以这里显式并入。
const isCNPlatformAccount = computed(
  () => isCNProviderPlatform(props.account?.platform ?? '') || props.account?.platform === 'opencode_go'
)

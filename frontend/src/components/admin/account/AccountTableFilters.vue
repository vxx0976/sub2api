...
// 平台清单取上游的单一目录（新增平台不会从这里静默消失），展示名走 fork 的品牌映射（zhipu → "GLM"）。
const pOpts = computed(() => [{ value: '', label: t('admin.accounts.allPlatforms') }, ...CONCRETE_PLATFORM_OPTIONS.map(o => ({ value: o.value as string, label: platformLabel(o.value) }))])

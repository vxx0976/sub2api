...
describe('admin UsageTable tooltip', () => {
  ...
  it('shows service tier and billing breakdown in cost tooltip', async () => {
    ...
    expect(text).toContain('$5 / 1M tokens')
    expect(text).toContain('$30 / 1M tokens')
  ...
  it('uses eight decimal places for missing cost values', async () => {
    ...
    // fork 的展示裁零约定（utils/formatters.ts 的 trimTrailingZeros）：0 渲染成 $0，
    // 不是上游那串 $0.00000000。精度提到 8 位只影响非零值的小数位数。
  ...
  it('hides mapped reasoning effort for user rows that only have the requested value', () => {
	...
	it.each([
		...
		const text = wrapper.text()
		// fork（713206b86）有意把模型列简化成「只展示实际上游模型」，不铺开
		// model_mapping_chain，所以请求模型 gpt-5.6-sol 不出现在这一列。
		// 上游本轮真正新增的是响应模型审计标记，那部分照常断言。

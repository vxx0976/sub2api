  ...
  <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
    ...
    <div class="card p-4 flex items-center gap-3">
      ...
      <div>
        ...
        <p class="text-xs">
          <span class="text-blue-600">{{ t('usage.read') }}: {{ formatTokens(stats?.total_cache_read_tokens || 0) }}</span> /
          <span class="text-amber-600">{{ t('usage.write') }}: {{ formatTokens(stats?.total_cache_creation_tokens || 0) }}</span>
        </p>
      </div>
    </div>
    <div class="card p-4 flex items-center gap-3">
      ...
      <div class="min-w-0 flex-1">
        ...
        <p class="text-xl font-bold text-green-600">
          ${{ trimTrailingZeros((stats?.total_actual_cost || 0).toFixed(4)) }}
        </p>
        <p class="text-xs text-gray-400">
          <template v-if="showAccountCost && totalAccountCost != null">
            <span class="text-orange-500">{{ t('usage.accountCost') }} ${{ trimTrailingZeros(totalAccountCost.toFixed(4)) }}</span>
            <span> · </span>
          </template>
          <span v-if="showStandardCost">
            {{ t('usage.standardCost') }}
            <span :class="{ 'line-through': strikeStandardCost }">${{ trimTrailingZeros((stats?.total_cost || 0).toFixed(4)) }}</span>
          </span>
        </p>
      </div>
    </div>

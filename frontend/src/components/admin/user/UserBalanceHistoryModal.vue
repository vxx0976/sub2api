...
const typeOptions = computed(() => [
  ...
  { value: 'recharge', label: t('admin.users.typeRecharge') },
...
// Helper: check if balance type (includes admin_balance, recharge, affiliate_balance)
const isBalanceType = (type: string) =>
  type === 'balance' || type === 'admin_balance' || type === 'recharge' || type === 'affiliate_balance'
...
const getItemTitle = (item: BalanceHistoryItem) => {
  switch (item.type) {
    ...
    case 'recharge':
      return t('redeem.balanceAddedRecharge')
    case 'concurrency':

    ...
    <table class="w-full min-w-[1000px] table-fixed border-collapse text-sm tabular-nums">
      ...
      <tbody>
        ...
        >
          ...
          >
            <!-- 上下文阶梯（上游）与官方峰谷档（fork）是两套正交信息：
                 有阶梯时按档展示，没有阶梯才拆高峰/空闲两行。 -->
 ...
 */
function symbolOf(m: PlazaModel): string {
  return costSymbol(m.price_currency)
}
...
/**
 * 实付价 = 渠道单价 × 生效倍率(时段行再乘时段倍率),按 <币种>/1M token 展示。
 * 币种逐行取:同一张表里国产 ¥ 与海外 $ 可以共存。
 */
function paidPerMillion(
  ...
  return formatScaled(value * rate, PER_MILLION, MIN_DECIMALS, symbolOf(m))
}

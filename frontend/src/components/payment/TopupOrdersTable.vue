        ...
        <tbody>
          ...
          >
            ...
            <td class="px-4 py-3">
              ...
              <span
                v-if="item.note"
                class="block max-w-[16rem] truncate text-xs text-gray-400 dark:text-gray-500"
                :title="item.note"
              >{{ item.note }}</span>
            </td>
            <!-- 用户（管理端） -->
            ...
            <!-- 到账（USD），手工扣减/退款为负值，按符号着色 -->
            <td class="px-4 py-3">
              <span class="text-sm font-medium" :class="creditClass(item)">{{ creditText(item) }}</span>
            </td>
            <!-- 状态 -->
            <td class="px-4 py-3">
            ...
            <td v-if="admin" class="px-4 py-3 text-right">
              <button
                v-if="item.status === 'paid' && item.channel !== 'manual'"
                type="button"
...
/** 把 (channel, pay_type) 归一化为面向用户的方式：alipay / wechat / usdt / manual */
function methodKind(item: MergedTopupOrder): 'alipay' | 'wechat' | 'usdt' | 'manual' | 'other' {
  if (item.channel === 'manual') return 'manual'
...
function methodLabel(item: MergedTopupOrder): string {
  ...
  if (kind === 'manual') return t('topupOrders.methodManual')
...
/** 金额展示口径：手工调整无"支付金额"显示 —；USDT 显示 USDT 数量，其余显示人民币（支付宝免签用实际支付金额） */
...
/** 到账金额文本：负值（手工扣减/退款）渲染为 -$50 而非 $-50。 */
function creditText(item: MergedTopupOrder): string {
  const v = item.credit_amount || '0'
  return v.startsWith('-') ? `-$${v.slice(1)}` : `$${v}`
}

/** 到账金额着色：正值（到账）绿色，负值（扣减）红色。 */
function creditClass(item: MergedTopupOrder): string {
  return (item.credit_amount || '').startsWith('-')
...
function statusLabel(status: string): string {
  switch (status) {
    ...
    case 'completed':
      return t('topupOrders.statusCompleted')
    default:
      return status
  }
}
